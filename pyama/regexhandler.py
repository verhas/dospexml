...
class RegexHandler(SegmentHandler):
    ...
    def postprocess(self, segment):
        ...
        trim_size = 0
        if trim:
            trim_size = self.max_line_length(segment, trim_size, trim_start)
            trim_size = self.calculate_trim_size(segment, trim_size, trim_start)
            self.trim_lines(segment, trim_size, trim_start)
    ...
    def trim_lines(self, segment, trim_size, trim_start):
        i = trim_start
        while i < len(segment.text) - 1:
    ...
    def calculate_trim_size(self, segment, trim_size, trim_start):
        i = trim_start
        while i < len(segment.text) - 1:
    ...
    def max_line_length(self, segment, trim_size, trim_start):
        i = trim_start
        while i < len(segment.text) - 1:
