...
class FileReader:
    def __init__(self, filename, regexes=[]):
        ...
        '''
        logger.debug("file reader for '%s' is using %s" % (filename,regexes))
    ...
    def _startsegment(self, line):
        ...
        :return: A tuple (match, name, regex) where
        match is False when there is no start regex matching the line and then the returned value is (False, None,None)
        When there is a match then the returned tuple is (True,name,regex), where
        name is the name of the segment, either from the first capture group or the next generated number
    ...
    def next_segment(self):
        """
        Create an artificial numbered segment if the segment is not named
    ...
    def read(self):
        ...
        with open(self.filename, 'r') as f:
            for line in f:
                ...
                if is_start:
                    logger.debug("line '%s' starts segment '%s' segment ending %s" % (line, name, end_regex))
